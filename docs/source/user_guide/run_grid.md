...
What it does NOT do:
- Does not expand the grid into combos
- Does not execute method scripts (no stage logs)
- Does not collect results or append new records into the summary table
...
bolt-grid --config /abs/path/to/your.yaml --output-dir ~/tmp/exp_001 --model-dir ~/code/bolt/pretrained_models --init-only
...
Step 3: (Recommended) Use `dry_run` to validate combo expansion and commands  
Set `dry_run=true` in YAML (see Section 3) to verify combo count and command construction before executing.

Step 4: Run with a new output directory  
For example:
...
Note:
- `dry_run` still goes through combo expansion and command construction, but does not run subprocesses.
- `--init-only` only initializes the workspace and does not expand combos.
