...
def create_datasets(tokenizer, data_args, training_args, apply_chat_template=False):
    ...
    if getattr(data_args, 'known_labels_file', None) and os.path.exists(data_args.known_labels_file):
        known_labels = pd.read_csv(data_args.known_labels_file, header=None)[0].tolist()
    else:
        known_labels = pd.read_csv(os.path.join(base_dir, 'label', f'label_{data_args.rate}.list'), sep='\t', header=None)[0].tolist()
    all_labels = pd.read_csv(os.path.join(base_dir, 'label', 'label.list'), sep='\t', header=None)[0].tolist()
    ...
    for split in split_set:
        ...
        tsv_path = os.path.join(base_dir, f'{split_name}.tsv')
        if not os.path.exists(tsv_path):
            tsv_path = os.path.join(base_dir, 'origin_data', f'{split_name}.tsv')
        df = pd.read_csv(tsv_path, sep='\t')
        if 'train' == split or 'train-semi' == split or 'dev' == split:
            labeled_path = os.path.join(base_dir, 'labeled_data', f'{split_name}_{data_args.labeled_ratio}.tsv')
            if not os.path.exists(labeled_path):
                bolt_path = os.path.join(base_dir, 'labeled_data', str(data_args.labeled_ratio), f'{split_name}.tsv')
                if os.path.exists(bolt_path):
                    bolt_df = pd.read_csv(bolt_path, sep='\t')
                    mask = bolt_df['labeled'].astype(str).str.strip().str.lower() == 'true'
                    labeld_df = df[mask.values].reset_index(drop=True)
                else:
                    labeld_df = df
            else:
                labeld_df = pd.read_csv(labeled_path, sep='\t')
