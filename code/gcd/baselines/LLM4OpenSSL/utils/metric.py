...
class Metrics():
    def __init__(self, data_args, model_args, training_args):
        data_dir = getattr(data_args, 'data_root_dir', '../data')
        base_dir = os.path.join(data_dir, data_args.dataset_name)

        if getattr(data_args, 'known_labels_file', None) and os.path.exists(data_args.known_labels_file):
            self.known_labels = pd.read_csv(data_args.known_labels_file, header=None)[0].tolist()
        else:
            self.known_labels = pd.read_csv(os.path.join(base_dir, 'label', f'label_{data_args.rate}.list'), header=None)[0].tolist()
        self.ori_labels = pd.read_csv(os.path.join(base_dir, 'label', 'label.list'), header=None)[0].tolist()
        ...
        labeled_train_path = os.path.join(base_dir, 'labeled_data', f'train_{data_args.labeled_ratio}.tsv')
        labeled_dev_path = os.path.join(base_dir, 'labeled_data', f'dev_{data_args.labeled_ratio}.tsv')
        if not os.path.exists(labeled_train_path):
            bolt_train_path = os.path.join(base_dir, 'labeled_data', str(data_args.labeled_ratio), 'train.tsv')
            bolt_dev_path = os.path.join(base_dir, 'labeled_data', str(data_args.labeled_ratio), 'dev.tsv')
            train_tsv = os.path.join(base_dir, 'origin_data', 'train.tsv') if os.path.exists(os.path.join(base_dir, 'origin_data', 'train.tsv')) else os.path.join(base_dir, 'train.tsv')
            dev_tsv = os.path.join(base_dir, 'origin_data', 'dev.tsv') if os.path.exists(os.path.join(base_dir, 'origin_data', 'dev.tsv')) else os.path.join(base_dir, 'dev.tsv')
            if os.path.exists(bolt_train_path):
                full_train = pd.read_csv(train_tsv, sep='\t')
                bolt_df = pd.read_csv(bolt_train_path, sep='\t')
                mask = bolt_df['labeled'].astype(str).str.strip().str.lower() == 'true'
                self.train_data = full_train[mask.values].reset_index(drop=True)
            else:
                self.train_data = pd.read_csv(train_tsv, sep='\t')
            if os.path.exists(bolt_dev_path):
                full_dev = pd.read_csv(dev_tsv, sep='\t')
                bolt_df = pd.read_csv(bolt_dev_path, sep='\t')
                mask = bolt_df['labeled'].astype(str).str.strip().str.lower() == 'true'
                self.dev_data = full_dev[mask.values].reset_index(drop=True)
            else:
                self.dev_data = pd.read_csv(dev_tsv, sep='\t')
        else:
            self.train_data = pd.read_csv(labeled_train_path, sep='\t')
            self.dev_data = pd.read_csv(labeled_dev_path, sep='\t')

        test_tsv = os.path.join(base_dir, 'test.tsv') if os.path.exists(os.path.join(base_dir, 'test.tsv')) else os.path.join(base_dir, 'origin_data', 'test.tsv')
        self.test_data = pd.read_csv(test_tsv, sep='\t')
