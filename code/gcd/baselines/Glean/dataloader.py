...
class Data:
    ...
    def __init__(self, args):
        ...
        if args.flag_demo or args.flag_demo_c:
            ...
            demo_data = (
                df_demo[df_demo["label"].isin(known_classes)]
                .groupby("label")
                .head(args.known_demo_num_per_class)
                .reset_index(drop=True)
            )
            demo_data_c = (
                df_demo[~df_demo["label"].isin(known_classes)]
                .groupby("label")
                .head(args.known_demo_num_per_class_c)
                .reset_index(drop=True)
            )
...
def convert_examples_to_features(
    args, examples, label_list, max_seq_length, tokenizer, mode=None
):
    label_map = {}
