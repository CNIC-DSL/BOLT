...
class ModelManager:
    def __init__(self, args, data, pretrained_model=None):
        ...
        self.model = CLBert(
            args, args.bert_model, device=self.device, num_labels=data.num_labels
        )
        ...
        args.num_training_rounds = math.ceil(
            args.num_train_epochs / args.update_per_epoch
        )
    ...
    def get_neighbor_dataset(
        self, args, data, indices, query_index, pred, p, cluster_name=None, init=False
    ):
        ...
        dataset = NeighborsDataset(
            ...
            di_all=self.di_all,
            di_all_pos_cluster_idx=self.di_all_pos_cluster_idx,
            di_all_neg_cluster_idx=self.di_all_neg_cluster_idx,
        )
    ...
    def get_neighbor_inds(self, args, data, km):
        ...
        indices, query_index, p = memory_bank.mine_nearest_neighbors(
    ...
    def eval_on_split(self, args, data, split="eval"):
        if split != "eval":
            raise NotImplementedError(
                "Only eval split is supported for early stopping."
            )
    ...
    def evaluation(self, args, data, save_results=True, plot_cm=True):
        ...
        feats_test, labels, logits = self.get_features_labels(
            data.test_dataloader, self.model, args, return_logit=True
        )
    ...
    def train(self, args, data):
        ...
        feats_gpu, labels, logits = self.get_features_labels(
            data.train_semi_dataloader, self.model, args, return_logit=True
        )
        ...
        es_metric = getattr(args, "es_metric", "NMI")
        ...
        for epoch in range(int(args.num_train_epochs)):
            ...
            for _, batch in enumerate(self.train_dataloader):
                ...
                with torch.set_grad_enabled(True):
                    ...
                    f_pos = torch.stack([out_an["features"], out_ng["features"]], dim=1)
                    ...
                    if args.weight_cluster_instance_cl > 0:
                        pos_cluster_idx_noisy = batch["pos_cluster_idx"]
                        neg_cluster_idx_noisy = batch["neg_cluster_idx"]

                        pos_cluster_idx = [
                            i for i in pos_cluster_idx_noisy if i is not None
                        ]
                        neg_cluster_idx = [
                            i for i in neg_cluster_idx_noisy if i is not None
                        ]

                        all_cluster_des_t = self.tokenizer(
                            cluster_name,
                            padding=True,
                            truncation=True,
                            return_tensors="pt",
                        ...
                        X_all_cluster_des = {
                            ...
                            "attention_mask": all_cluster_des_t["attention_mask"].to(
                                self.device
                            ),
                            "token_type_ids": all_cluster_des_t["token_type_ids"].to(
                                self.device
                            ),
                        }
                        ...
                        an_w_cluster_des_feat = an_w_cluster_des_feat[
                            [
                                i
                                for i in range(len(pos_cluster_idx_noisy))
                                if pos_cluster_idx_noisy[i] is not None
                            ]
                        ]

                        pos_cluster_feat = [
                            feat_all_cluster_des[i] for i in pos_cluster_idx
                        ]
                        neg_cluster_feat = [
                            feat_all_cluster_des[i] for i in neg_cluster_idx
                        ]
                        ...
                        if len(an_w_cluster_des_feat) > 0 and len(pos_cluster_feat) > 0:
                            for i in range(len(an_w_cluster_des_feat)):
                                ...
                                sim_positive = (
                                    F.cosine_similarity(
                                        an_w_cluster_des_feat[i].unsqueeze(0),
                                        pos_cluster_feat[i].unsqueeze(0),
                                        dim=1,
                                    )
                                    / args.temp
                                )
                    ...
                    if args.weight_ce_unsup > 0:
                        ...
                        student_out = torch.cat(
                            [out_an["logits"], out_an_2["logits"]], dim=0
                        )
                        teacher_out = student_out.detach()

                        cluster_loss = cluster_criterion(
                            student_out, teacher_out, epoch
                        )
                        avg_probs = (student_out / 0.1).softmax(dim=1).mean(dim=0)
                        me_max_loss = -torch.sum(
                            torch.log(avg_probs ** (-avg_probs))
                    ...
                    loss = (
                        loss_ce * args.ce_weight
                        + loss_cl * args.cl_weight
                        + loss_cl_cluster_instance * args.weight_cluster_instance_cl
                    )
                    pstr += f"sup_ce_loss: {loss_ce_sup.item():.2f} "
                    pstr += f"\t loss_ce: {loss_ce.item():.2f} "
                    pstr += f"loss_cl: {loss_cl.item():.2f} "
                    pstr += (
                        f"loss_cl_cluster_instance: {loss_cl_cluster_instance.item():.2f} "
                        if loss_cl_cluster_instance != 0
                        else ""
                    )
            ...
            km_eval = KMeans(n_clusters=data.n_known_cls, random_state=args.seed).fit(
                feats_eval_np
            )
            ...
            print(
                f"[ES] epoch={epoch} {monitor}={es_score:.2f} (best={best_es_score:.2f})"
            )
            ...
            if es_score > best_es_score + es_min_delta:
                ...
                if es_wait >= es_patience:
                    print(
                        f"[ES] Stop main training early at epoch {epoch+1}. Best {monitor}={best_es_score:.2f}"
                    )
    ...
    def category_characterization(self, data, km, feats_gpu):
        ...
        if self.args.interpret_sampling_strategy == "nearest_center":
            ...
            index = [
                torch.randperm(len(i))[:interpret_num_representatives] for i in index
            ]
            ...
            for i in range(self.num_labels):
                ...
                sub_km = KMeans(
                    n_clusters=interpret_num_representatives,
                    random_state=self.args.seed,
                ).fit(cluster_feats.cpu().numpy())
                sub_cluster_centers = torch.tensor(sub_km.cluster_centers_).to(
                    feats_gpu.device
                )
            ...
            raise NotImplementedError(
                f"Sampling strategy {self.args.interpret_sampling_strategy} not implemented!"
            )
                ...
                for j in index[i]:
                    query_text.append(
                        self.tokenizer.decode(
                            data.train_semi_dataset.__getitem__(j)[0],
    ...
    def query_llm(self, query, example_count):
        ...
        utterances = [
            self.tokenizer.decode(
                utt, skip_special_tokens=True, clean_up_tokenization_spaces=True
            )
            for utt in query
        ]
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    if attempt < max_retries - 1:
                        print(
                            f"LLM API call attempt {attempt + 1} failed: {e}. Retrying in {retry_delay} seconds..."
                        )
                        time.sleep(retry_delay)
                    else:
                        print(
                            f"LLM API call attempt {attempt + 1} failed: {e}. No more retries left."
                        )
    ...
    def save_results(self, args, result_source=None):
        ...
        results["args"] = results["args"].apply(
            lambda x: {i: v for i, v in x.items() if "prompt_demo" not in i}
        )
...
if __name__ == "__main__":
    ...
    var = [
        ...
        args.cl_weight,
        args.sup_weight,
        args.weight_ce_unsup,
        args.options,
        args.query_samples,
        args.update_per_epoch,
        args.sampling_strategy,
        args.allocation_degree,
        args.weight_cluster_instance_cl,
        args.options_cluster_instance_ratio,
    ]
    names = [
        "dataset",
        "running_method",
