...
class MemoryBank(object):
    ...
    def mine_nearest_neighbors(self, topk, y_pred, cluster_centers_):
        ...
        try:
            ...
            print(f"Faiss GPU index failed or not available, using CPU index. Error: {e}")
        index.add(features)
        ...
        if self.args.sampling_strategy == 'loop':
            ...
            inx_list = inx[:self.args.query_samples].cpu().tolist()
            index_list = index[:self.args.query_samples].cpu().tolist()
            ...
            query_index = inx[:self.args.query_samples].cpu().tolist()
