...
class PretrainModelManager:
    ...
    def train(self):
        ...
        for epoch in tqdm(range(int(self.args.num_pretrain_epochs)), desc="Pretraining Epochs"):
            self.model.train()
            tr_loss = 0
            ...
            print(f'Pretrain Epoch {epoch+1}/{int(self.args.num_pretrain_epochs)} - Loss: {loss:.6f} - Eval Acc: {eval_score:.2f}% - Best: {self.best_eval_score:.2f}% - Wait: {wait}/{self.args.wait_patient}')

            if eval_score > self.best_eval_score:
                best_model = copy.deepcopy(self.model)
                wait = 0
                self.best_eval_score = eval_score
                print(f'  → New best model! Eval Acc improved to {eval_score:.2f}%')
            else:
                wait += 1
        ...
        print(f'\nPretraining completed. Best eval accuracy: {self.best_eval_score:.2f}%')
        if self.args.save_model:
            self.save_model()
            print(f'Model saved to {self.args.pretrain_dir}')
