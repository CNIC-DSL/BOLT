...
class Data:
    ...
    def __init__(self, args):
        ...
        max_seq_lengths = {'clinc':30, 'stackoverflow':45, 'banking':55, 'hwu':55, 'ecdt':65, 'thucnews':256}
        train_epochs = {'clinc':20, 'stackoverflow':10, 'banking':20, 'hwu':20, 'ecdt':20, 'thucnews':20}
        args.max_seq_length = max_seq_lengths.get(args.dataset, 55)
        args.num_train_epochs = train_epochs.get(args.dataset, 20)
    ...
    def get_examples(self, processor, args, mode = 'train'):
        ...
        if mode == 'train':
            # Use labeled samples from file if available (consistent with other baselines)
            if self.known_train_sample is not None:
                known_samples_set = set(zip(
                    self.known_train_sample['text'].astype(str),
                    self.known_train_sample['label'].astype(str)
                ))
                train_labeled_examples, train_unlabeled_examples = [], []
                for example in ori_examples:
                    if (example.text, example.label) in known_samples_set:
