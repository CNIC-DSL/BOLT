...
from methods.alup.utils_prompt import PROMPT_BANKING, PROMPT_CLINC, PROMPT_STACKOVERFLOW, PROMPT_GCD
from methods.alup.utils_LLM import chat_completion_with_backoff, OPENAI_PIRCE, get_global_token_usage
...
class ALManager:
    ...
    def prepare_data(self, args, data):
        ...
        # llm_labels: stores LLM-annotated labels; used for selecting samples for LLM annotation and label propagation
    ...
    def al_finetune(self, args):
        ...
        for epoch in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            if results['ACC_all'] + results['ARI'] + results['NMI'] > \
                best_metrics['ACC_all'] + best_metrics['ARI'] + best_metrics['NMI']:
                best_metrics['Epoch'] = epoch
                best_metrics['ACC_all'] = results['ACC_all']
            ...
            self.logger.info(
                ...
                str(epoch), str(results['ACC_all']), str(results['ARI']), str(results['NMI'])
            )
            self.logger.info(
                "Best Epoch %s Test Score: ACC = %s | ARI = %s | NMI = %s",
                str(best_metrics['Epoch']), str(best_metrics['ACC_all']), str(best_metrics['ARI']), str(best_metrics['NMI'])
            )
    ...
    def llm_labeling(self, args, epoch, model):
        ...
        if not os.path.exists(os.path.join(args.result_dir, f'llm_annotated_output_{args.seed}_{args.known_cls_ratio}_{epoch}.json')):
            ...
            # Hungarian alignment: y_pred_map maps each sample's predicted label; cluster_map maps each cluster center
            y_pred_map, cluster_map, cluster_map_opp = self.get_hungray_aligment(y_pred, y_true)
            # Select the nearest sample to each centroid for LLM querying; returns [num_centroids, 1] indices in train_semi_dataset
            ...
            # uncertainties: (selected_num_samples,) entropy for each sample
            # unc_neighbors: (selected_num_samples, num_neighbours) nearest neighbor indices for label propagation
            # probs: (selected_num_samples, num_clusters) cluster assignment probabilities
            # sim_scores: (selected_num_samples, num_neighbours) similarity scores of nearest neighbors
            ...
            # Index for finding nearest clusters for the most uncertain samples
            ...
            for i, cluster_ind in tqdm(enumerate(np.unique(y_pred)), desc="LLM Labeling Iter", total=len(np.unique(y_pred)), leave=False, dynamic_ncols=True):
                ...
                # Get representative examples for these clusters
                ...
                for j, indice in enumerate(cluster_representative_ex_inds):
                    ...
                    # Verify labels are consistent (no label leakage)
                    assert y_true[indice] == self.train_semi_dataset[indice]['label_id_true']
                    if args.dataset == 'thucnews':
                        utterance_set += f'Cluster_id: {cluster_representative_ex_labs[j]}. Sample: {utterenace_text}\n'
                    else:
                ...
                if args.dataset == 'banking':
                    ...
                    comparison_prompting = PROMPT_GCD.format(utterance_set, max_uncertain_utterance_text)
                ...
                price = OPENAI_PIRCE.get(actual_model, {'input_token': 0.0, 'output_token': 0.0})
                curr_usage = num_input_tokens * price['input_token'] + num_output_tokens * price['output_token']
                ...
                for neighbor_ind, neighbor_sim_score in zip(max_unex_unc_neighbors, max_unex_sim_scores):
                    if llm_pred_cluster_id == -1:
                        ...
                        # Original (unmapped) cluster order
                        neighbor_prob = probs[neighbor_ind]
                        # Reverse-map llm_pred_cluster_id to original cluster order
                        ...
                        # Select the highest probability; map back since it is in original order
    ...
    def print_token_usage(self):
        usage = get_global_token_usage()
        self.logger.info("=" * 40)
        self.logger.info("Global Token Usage Summary:")
        self.logger.info(f"   Input (Prompt): {usage['prompt_tokens']}")
        self.logger.info(f"   Output (Completion): {usage['completion_tokens']}")
    ...
    def get_nearst_centroid_example(self, feats, cluster_centroids, cluster_map, num_examples=1):
        self.logger.info('Get utterances nearest to centroids')
    ...
    def get_uncertainty(self, args, feats, cluster_centroids):
        ...
        st_distribution = np.clip(st_distribution, 1e-12, 1.0) # Prevent NaN from boundary values
        # Get entropies
    ...
    def test(self, test_dataloader, model, num_labels):
        ...
        test_results = clustering_score(y_true, y_pred, self.known_lab)
