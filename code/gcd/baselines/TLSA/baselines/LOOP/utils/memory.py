...
class MemoryBank(object):
    ...
    def mine_nearest_neighbors(self, topk, y_pred, cluster_centers_):
        ...
        # Use CPU version (GPU version has compatibility issues)
        # CPU faiss is fast enough for nearest neighbor search
        print(f"Using FAISS CPU version for {n} samples with dim={dim}")
        ...
        query_index = []
        # Convert to list of python ints to ensure compatibility with dataloader index
        inx_list = inx[:500].cpu().tolist()
        index_list = index[:500].cpu().tolist()

        for i in index_list:
            if i in inx_list:
                query_index.append(int(i))
    ...
    def entropy(self, x, eps=1e-5):
        p = F.softmax(x, dim=-1)
        entropy = -torch.sum(p * torch.log(p + eps), 1)
        return entropy
