...
def clustering_accuracy_score(y_true, y_pred, known_lab):
    ...
    # Harmonic mean score (H-Score)
    h_score = 2 * old_acc * new_acc / (old_acc + new_acc) if (old_acc + new_acc) > 0 else 0
    return (round(acc*100, 2), round(old_acc*100, 2), round(new_acc*100, 2), round(h_score*100, 2), round(nmi*100, 2), round(ari*100, 2))

def clustering_score(y_true, y_pred, known_lab):
    Acc, Known, Novel, HScore, NMI, ARI = clustering_accuracy_score(y_true, y_pred, known_lab)
    return {'Acc': Acc,
            'Known': Known,
            'Novel': Novel,
            'H-Score': HScore,
