...
from utils.token_tracker import update_token_usage, print_token_usage
...
class LoopModelManager:
    def __init__(self, args, data, pretrained_model=None):
        ...
        # Free memory of pretrained model as it is no longer needed if cluster_num_factor is 1
        if args.cluster_num_factor <= 1:
            del self.pretrained_model
            self.pretrained_model = None
            torch.cuda.empty_cache()
            print("[INFO] Pretrained model deleted to save memory.")
    ...
    def get_neighbor_dataset(self, args, data, indices, query_index, pred):
        dataset = NeighborsDataset(args, data.train_semi_dataset, indices, query_index, pred, llm_cache=self.llm_cache)
        self.train_dataloader = DataLoader(dataset, batch_size=args.train_batch_size, shuffle=True)
        self.dataset = dataset
        # Update manager cache with results from dataset (which handles pre-querying)
        self.llm_cache = dataset.di
    ...
    def get_neighbor_inds(self, args, data, km):
        ...
        print(f"DEBUG: Selected {len(query_index)} samples for LLM query (Active Learning)")
    ...
    def train(self, args, data):
        ...
        for epoch in range(int(args.num_train_epochs)):
            ...
            print(f'LLM Queries in this epoch: {self.dataset.count}')
    ...
    def query_llm(self, a):
        ...
        if self.args.dataset == 'thucnews':
            prompt = "Given the following Chinese news titles/content, return a word or a phrase to summarize the common topic of these texts without explanation. \n Text 1: " + s1 + "\n Text 2: " + s2 + "\n Text 3: " + s3
        else:
        ...
        # Use langchain with environment variables for API configuration
        ...
        model = os.getenv('LLM_MODEL') or getattr(self.args, 'llm_model_name', '') or 'gpt-3.5-turbo'

        if not api_key or api_key == 'EMPTY':
            print("Warning: LLM API key not found. Skipping LLM query.")
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt < max_retries - 1:
                    print(f"LLM naming query attempt {attempt + 1} failed: {e}. Retrying in {retry_delay}s...")
                    time.sleep(retry_delay)
                    retry_delay *= 2 # Exponential backoff
                else:
                    print(f"LLM naming query failed after {max_retries} attempts: {e}")
                    traceback.print_exc()
