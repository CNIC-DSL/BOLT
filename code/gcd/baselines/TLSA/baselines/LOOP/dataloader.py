...
max_seq_lengths = {'clinc':30, 'stackoverflow':45, 'banking':55, 'thucnews': 256}
TOPK = {'clinc':50, 'stackoverflow':500, 'banking':50, 'thucnews': 50}
class Data:
    ...
    def __init__(self, args):
        ...
        self.all_label_list = np.array([str(item) for item in self.all_label_list])
        self.n_known_cls = round(len(self.all_label_list) * args.known_cls_ratio)

        # Load known labels from label file if exists
        ...
        elif self.n_known_cls > 0:
            self.known_label_list = list(np.random.choice(self.all_label_list, self.n_known_cls, replace=False))
        else:
            self.known_label_list = []

        # Build known_lab: indices of known labels in all_label_list
        self.known_lab = [int(np.where(self.all_label_list == lab)[0][0]) for lab in self.known_label_list if lab in self.all_label_list]
    ...
    def get_semi(self, labeled_examples, unlabeled_examples, args):
        ...
        if self.n_known_cls > 0:
            # IMPORTANT: Use all_label_list to ensure consistent label indices
            labeled_features = convert_examples_to_features(labeled_examples, self.all_label_list, args.max_seq_length, tokenizer)
