...
class MultiCEFocalLoss(torch.nn.Module):
    ...
    def forward(self, predict, target):
        pt = F.softmax(predict, dim=1) # softmax to get prediction probabilities
        class_mask = F.one_hot(target, self.class_num) # get one-hot encoding of target
        ids = target.view(-1, 1)
        alpha = self.alpha[ids.data.view(-1)] # per-class weight factors
        probs = (pt * class_mask).sum(1).view(-1, 1) # use one-hot as mask to extract corresponding pt
        log_p = probs.log()
        # add dynamic weight decay factor on top of standard CE
