...
class K_Means:
    ...
    def predict(self,X):
        dist = pairwise_distance(X, self.cluster_centers_, self.pairwise_batch_size)
    ...
    def topk(self,k,features):
        centers=self.cluster_centers_
        id_all=torch.zeros((self.k,k),dtype=torch.long)
        dist = pairwise_distance(centers, features, self.pairwise_batch_size)
        for i in range(self.k):
            dis_i=dist[i]
            _,id=torch.topk(dis_i,k,largest=False)
            id_all[i]=id
        return id_all
