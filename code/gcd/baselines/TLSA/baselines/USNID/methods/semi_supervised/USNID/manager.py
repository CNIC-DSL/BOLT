...
class USNIDManager:
    ...
    def train(self, args, data):
        ...
        # Save final model weights to an easy-to-find path
        weights_dir = os.path.join(args.method_output_dir, 'final_weights')
        if not os.path.exists(weights_dir):
            os.makedirs(weights_dir)
        weight_name = f"USNID_{args.dataset}_kcr{args.known_cls_ratio}_seed{args.seed}.pt"
        weight_path = os.path.join(weights_dir, weight_name)
        model_to_save = self.model.module if hasattr(self.model, 'module') else self.model
        torch.save(model_to_save.state_dict(), weight_path)
        self.logger.info('Final model weights saved to %s', weight_path)
