...
def get_examples(args, base_attrs, mode):
    ...
    if args.setting == 'unsupervised':
        ...
        if mode == 'train':
            known_train_sample = pd.read_csv(f"{base_attrs['data_dir']}/labeled_data/train_{args.labeled_ratio}.tsv", sep='\t')
            known_train_sample = known_train_sample[known_train_sample['label'].isin(base_attrs['known_label_list'])]

            train_labeled_examples, train_unlabeled_examples = [], []
            for idx, example in enumerate(ori_examples):
                if example.text_a in known_train_sample['text'].tolist() and example.label in known_train_sample['label'].tolist():
                    train_labeled_examples.append(example)
                else:
                    train_unlabeled_examples.append(example)
            return ori_examples, train_labeled_examples, train_unlabeled_examples
...
def get_semi_loader(labeled_examples, unlabeled_examples, base_attrs, args, tokenizer):
    ...
    semi_dataloader = DataLoader(semi_data, sampler=semi_sampler, batch_size = args.train_batch_size, num_workers = args.num_workers, pin_memory = True)
