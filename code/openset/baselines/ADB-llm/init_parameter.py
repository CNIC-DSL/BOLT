...
def init_model():
    ...
    parser.add_argument(
        "--output_dir",
        type=str,
        default="./outputs/openset/adb",
        help="The unified output directory for all experiment artifacts.",
    )
    ...
    parser.add_argument(
        "--save_model", type=str_to_bool, default=False, help="save trained-model"
    )
    parser.add_argument(
        "--save_results", type=str_to_bool, default=False, help="save test results"
    )
    ...
    parser.add_argument(
        "--lr", default=2e-5, type=float, help="The learning rate of BERT."
    )
    parser.add_argument(
        "--cluster_num_factor",
        default=1.0,
    ...
    parser.add_argument(
        "--wait_patient", default=10, type=int, help="Patient steps for Early Stop."
    )
    parser.add_argument(
        "--lr_boundary",
        type=float,
        default=0.05,
        help="The learning rate of the decision boundary.",
    )
    ...
    parser.add_argument(
        "--llm_ood", action="store_true", help="Enable LLM OOD check during test"
    )
    parser.add_argument(
        "--llm_api_base",
        type=str,
    ...
    parser.add_argument(
        "--llm_model", type=str, default="gpt-4o-mini", help="LLM model name"
    )
    parser.add_argument(
        "--llm_api_key_env",
        type=str,
    ...
    parser.add_argument(
        "--llm_batch", type=int, default=8, help="LLM batch size (prompt-wise)"
    )
    parser.add_argument("--llm_temperature", type=float, default=0.0)
    parser.add_argument(
        "--llm_cache",
        type=str,
        default="llm_cache.jsonl",
