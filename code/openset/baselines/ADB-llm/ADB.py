    ...
    def evaluation(self, args, data, mode="eval"):
        ...
        for batch in tqdm(dataloader, desc="Iteration"):
            ...
            with torch.set_grad_enabled(False):
                ...
                logits = euclidean_metric(pooled_output, self.centroids)
                _, preds_before = F.softmax(logits.detach(), dim=1).max(dim=1)
                euc_dis = torch.norm(pooled_output - self.centroids[preds_before], 2, 1)
                deltas_pred = self.delta[preds_before]
                margin = euc_dis - deltas_pred
        ...
        if mode == "eval":
            ...
            texts = [
                getattr(ex, "text", None) or getattr(ex, "text_a", "")
                ...
                rows.append(
                    {
                        ...
                        "label": labels[i],
                        "llm_pred": llm_pred[i],
                        "abd_pred": int(abd_pred[i]),
                        "ood_score": float(ood_margins[i]),
                    }
                )
            ...
            pd.DataFrame(
                rows, columns=["text", "label", "llm_pred", "abd_pred", "ood_score"]
...
def llm_ood_eval(args, data, test_examples, known_labels, output_dir):
    ...
    if batch:
        outs = llm_chat_batch(
            args.llm_api_base, args.llm_model, api_key, batch, args.llm_temperature
        )
        for pm, out in zip(batch, outs):
            kk = _hash(pm[-1]["content"])
            _append_cache(cache_path, kk, {"response": out})
