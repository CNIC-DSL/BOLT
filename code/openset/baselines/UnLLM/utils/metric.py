...
class Metrics():
    def __init__(self, data_args, model_args, training_args):
        if getattr(data_args, 'known_labels_file', None) and os.path.exists(data_args.known_labels_file):
            id_list = pd.read_csv(data_args.known_labels_file, header=None)[0].tolist()
        else:
        ...
        label_list_path = os.path.join(data_dir, data_args.dataset_name, 'origin_data', 'label.list')
        if os.path.exists(label_list_path):
            self.target_names = pd.read_csv(label_list_path, header=None)[0].tolist()
        else:
            all_labels = set()
            for split_file in ['train.tsv', 'dev.tsv', 'eval.tsv', 'test.tsv']:
                fpath = os.path.join(data_dir, data_args.dataset_name, 'origin_data', split_file)
                if os.path.exists(fpath):
                    df = pd.read_csv(fpath, sep='\t')
                    all_labels.update(df['label'].dropna().unique())
            self.target_names = sorted(all_labels)
